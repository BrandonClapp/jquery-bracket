...
($ => {
  ...
  function parseOptions<TTeam, TScore>(
    ...
    const opts: Options<TTeam, TScore> = {
      ...
      onMatchClick: input.onMatchClick ? input.onMatchClick : () => { return; },
      onMatchHover: input.onMatchHover ? input.onMatchHover : () => { return; },
  ...
  function parseInit<TTeam, TScore>(
    ...
    const value: BracketInitData<TTeam, TScore> = rawInit ? rawInit : {
      results: [],
      teams: [[null, null]]
    };
  ...
  function init<TTeam, TScore>(
    ...
    const internalOpts = parseOptions<TTeam, TScore>(originalOpts, ctx);
  ...
  $.fn.bracket = function(
    ...
    if (typeof method === "string" && method === "data") {
      ...
      return init<string, number>(this, {
        ...options,
        _internal: { evaluateScore: defaultEvaluateScore },
